...
function insert(string, string2) {
...
function viewCache() {
	chrome.storage.local.get(['Testing'], function(result) {
		...
		for (var i = 0; i < shuffled.length - 1; i++) {
			initial = insert(initial, shuffled[i + 1][0]);
		}
...
function infoCache() {
	chrome.storage.local.get(['Testing'], function(result) {
		...
		for (var i = 0; i < result.Testing.length; i++) {
			listOfSites +=  "<div><label><input type='checkbox' value='' class='checkbox' id='" + i + "'>" + result.Testing[i][1] + "</label></div><br>";
		}
...
$(document).keyup(function(e) {
     if (e.keyCode == 72) {
     	if ($('#optionsContainer_sw').is(':visible') && $('#cache_sw').hasClass('cache_sw')) {
        	$('#optionsContainer_sw').hide();
     	} else {
     		$('#optionsContainer_sw').show();
     	}
     	console.log("pressed");
